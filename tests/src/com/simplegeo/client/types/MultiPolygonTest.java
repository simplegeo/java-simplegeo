...
public class MultiPolygonTest extends TestCase {
	...
	public void testToJSONArray() {
		try {
			...
			this.assertEquals(5, jsonArray.getJSONArray(0).length());
			this.assertEquals(5, jsonArray.getJSONArray(1).length());
	...
	public void testFromJSONArray() {
		try {
			...
			this.assertEquals(2, multiPolygon.getPolygons().size());
			this.assertEquals(1, multiPolygon.getPolygons().get(0).getRings().size());
			this.assertEquals(2, multiPolygon.getPolygons().get(1).getRings().size());
		} catch (JSONException e) {
			this.fail(e.getMessage());
		}
	}
